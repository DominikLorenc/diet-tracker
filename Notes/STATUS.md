...
- ~~**B13 — `Decimal` + surowy JS `number` = dryf floating-point**~~ ✅ ZROBIONE (2026-07-28) — wpisanie `fat: 9.7` w `ProductForm` wracało jako `"9.699999999999999"`. Przyczyna: Prisma konwertuje JS `number` na swój `Decimal` o dowolnej precyzji wprost z binarnej reprezentacji IEEE-754 double — a ta nie jest w stanie dokładnie zapisać 9.7. `console.log(9.7)` pokazuje `"9.7"` tylko dlatego, że silnik JS wybiera *najkrótszy* string, który po sparsowaniu daje tę samą liczbę; Prisma tego skrótu nie robi. Fix: `toDecimalSafe()` w `productService.ts` — stringuje (`.toString()`, czyli tym samym „ładnym" algorytmem JS) pola `calories/carbs/protein/fat/gramsPerUnit` *zanim* trafią do `prisma.product.create/update`. **Dług:** naprawione tylko w `productService.ts` — ten sam bug może siedzieć wszędzie indziej, gdzie zapisujemy `Decimal` z JS `number` (`RecipeIngredient.quantity`, `UserRecipeIngredient.quantity`, `DiaryEntryItem.quantity`, `BodyMeasurement`).
