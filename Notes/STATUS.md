...
> Jedyne źródło prawdy o stanie projektu. Zweryfikowane z kodem 2026-06-03. Ostatnia aktualizacja: 2026-07-28.
...
- **Products:** CRUD, wyszukiwanie, integracja Open Food Facts (cache-aside), `imageUrl`, `barcode`, `gramsPerUnit` (2026-07-28) — nullable Decimal, opcjonalna waga „1 sztuki" (np. jajko = 60g), pozwala dodawać produkt do dziennika w sztukach zamiast tylko w gramach. **Świadomie tylko `Product`** — `Recipe`/`UserRecipe` nadal wyłącznie w gramach (ich logika przelicznika porcji, `calcRecipeMacros`, zostawiona nietknięta; YAGNI, brak zgłoszonej potrzeby).
...
| `/dashboard/add`               | ✅            | dodawanie wpisu, ProductSearch, RecipeSearch; `ProductCard` ma przełącznik g/szt. gdy produkt ma `gramsPerUnit` — do dziennika zawsze leci przeliczone na gramy (2026-07-28) |
| `/dashboard/products`          | ✅            | lista + ProductForm (formularz ma pole „Waga sztuki (g, opcjonalne)", 2026-07-28)           |
...
- ~~`CORS_ORIGINS` na Render nieużywane~~ ✅ ZROBIONE (2026-07-17) — zmienna usunięta z Rendera. Była martwa, bo ruch Vercel→Render idzie **server-side** (rewrite proxy), a nie z przeglądarki — CORS to mechanizm przeglądarki, więc nikt nigdy nie pytał o te nagłówki. Uwaga: fallback na localhost w `app.ts` **zostaje** i jest potrzebny — lokalnie front (`:3000`) gada z backendem (`:4000`) wprost z przeglądarki, czyli cross-origin.
...
- ~~Testowy user `rendertest@example.com` w prod bazie~~ ✅ ZROBIONE (2026-07-17) — usunięty z prod bazy.
...
- ~~**B3 — Usuń `console.log` z produkcji**~~ ✅ ZAMKNIĘTE (2026-07-17) — dwuczęściowe. **(a) Sprzątanie: bezprzedmiotowe** (zweryfikowane 2026-07-15) — wskazane `console.log` (`productController` ~99, `diaryService` ~134/153) **już nie istnieją**. W całym `src` zostały tylko dwa `console.`: `index.ts:9` (log startu serwera, standard) i `errorHandler.ts:12` (`console.error` w handlerze — właściwe miejsce). **(b) Decyzja „czy `pino`": NIE — zostajemy przy `console`** (2026-07-17). Powód: `pino` kupuje strukturalny JSON (grepowalny po polach), poziomy logowania i wydajność przy tysiącach logów/s — a my mamy **dwa** wywołania `console.` i 2 userów. Render i tak zbiera stdout, a przy tej objętości czyta się go oczami, nie zapytaniem. Zysk zerowy, koszt = zależność + konfiguracja + transport do ładnego formatu w devie. **Kiedy wrócić do tematu:** gdy pojawi się realna potrzeba przeszukiwania logów (więcej userów, debug incydentu po fakcie) albo zewnętrzny agregator (Better Stack, Datadog) — te narzędzia jedzą JSON, nie zdania. Wtedy `pino` staje się opłacalny, dziś to over-engineering. **Lekcja (ta sama co B5/B10):** notatka TODO się zdezaktualizowała — zweryfikuj grepem realny stan, zanim „naprawisz".
...
- ~~**B8 — Spójny klucz błędów**~~ ✅ ZROBIONE (2026-07-15) — okazało się, że wyłamywało się **dokładnie jedno** miejsce: `productController.ts:25` zwracał `{ errors: ... }`, cała reszta API (kilkadziesiąt miejsc) używa `{ message: ... }`. Zmiana `errors` → `message`. Zweryfikowane grepem: `errors:` nie występuje już nigdzie w kontrolerach/middleware, żaden test nie asertował na tym kluczu. 72/72.
- ~~**B9 — Walidacja JWT payload**~~ ✅ ZROBIONE (2026-07-15) — `authMiddleware.ts` nie robi już `verifyToken(token) as jwt.JwtPayload` (ślepe rzutowanie = obietnica dla kompilatora, zero sprawdzenia runtime → `undefined` w `req.userId` cichnie i wybucha 5 warstw dalej). Teraz payload przechodzi przez `JWTSchema.safeParse` (`schemas/userSchema.ts`: `id: z.uuid()`, `role: z.enum(['ADMIN','USER'])`), `!success → 401`. Schemat **świadomie ostry** (a nie `z.string()`) — decyzja usera. **Efekt uboczny = lekcja:** 47 testów padło, bo podpisywały tokeny `{ id: '1' }` bez `role` — „działały" TYLKO dzięki łatanej dziurze (ślepy `as` przepuszczał śmieci). Poprawione na realistyczne tokeny (`crypto.randomUUID()` + `role`) w 5 plikach; żadna asercja `toHaveBeenCalledWith` nie wiązała id z tokenu, więc wystarczyła podmiana linii `jwt.sign`. 72/72, `tsc` czysto.
- ~~**B13 — `Decimal` + surowy JS `number` = dryf floating-point**~~ ✅ ZROBIONE (2026-07-29, dług domknięty) — wpisanie `fat: 9.7` w `ProductForm` wracało jako `"9.699999999999999"`. Przyczyna: Prisma konwertuje JS `number` na swój `Decimal` o dowolnej precyzji wprost z binarnej reprezentacji IEEE-754 double — a ta nie jest w stanie dokładnie zapisać 9.7. `console.log(9.7)` pokazuje `"9.7"` tylko dlatego, że silnik JS wybiera *najkrótszy* string, który po sparsowaniu daje tę samą liczbę; Prisma tego skrótu nie robi. Fix (2026-07-28): `toDecimalSafe()` w `productService.ts` — stringuje (`.toString()`, czyli tym samym „ładnym" algorytmem JS) pola `calories/carbs/protein/fat/gramsPerUnit` *zanim* trafią do `prisma.product.create/update`. **Dług domknięty (2026-07-29):** `toDecimalSafe` wyniesiony do współdzielonego `backend/src/utils/toDecimalSafe.ts` (generyczny, przyjmuje listę pól: `toDecimalSafe(data, fields)`), `productService.ts` przepisany na wersję współdzieloną. Zastosowany też w pozostałych miejscach, gdzie surowy JS `number` leciał w pole `Decimal`: `recipeService.ts` (`RecipeIngredient.quantity`, create + update), `userRecipeService.ts` (`UserRecipeIngredient.quantity`, create + update; `copyRecipe` pominięty świadomie — tam `quantity` już jest `Prisma.Decimal` z odczytu z bazy, nie surowy `number`), `diaryService.ts` (`DiaryEntryItem.quantity`, oba miejsca tworzenia wpisu), `measurementsService.ts` (`BodyMeasurement.weight/waist/hips/arm`, create + update). Zweryfikowane: `tsc --noEmit` czysto, 84/84 testów.
- ~~**B11 — Serwis zwraca więcej niż deklaruje kontrakt**~~ ✅ ZROBIONE (2026-07-29) — pierwotnie `updateGoalsService` zwracał **całego `User`** (`User & { userGoals }`, z hashem hasła w komplecie) przez nested write (`prisma.user.update({ data: { userGoals: { upsert: {...} } }, include: { userGoals: true } })`), a dok OpenAPI i wygenerowany typ we froncie deklarowały `updated` jako **sam `UserGoal`**. Doraźna łatka z 2026-07-03 (kontroler odsyłał `updated.userGoals`) zastąpiona fixem u źródła: serwis teraz robi `prisma.userGoal.upsert({ where: { userId }, update: {...}, create: { userId, ...} })` bezpośrednio na modelu `UserGoal` (ma `userId @unique`) — zero dotykania `User`, zwraca `Promise<UserGoal>` wprost, zgodnie z kontraktem. Kontroler: `res.json({ message, updated })` bez wycinania `.userGoals`. Zaktualizowany mock w `user.test.ts` (płaski `UserGoal`, nie zagnieżdżony `User`). `tsc --noEmit` czysto, 84/84 testów. **Zostaje jako mniejszy dług:** (b) niespójna konwencja odpowiedzi mutacji — PATCH goals zwraca `updated`, GET `/users/me` zwraca `user` — nieujednolicone w całym API (pokrewne B8).
...
- ~~**B12 — `clearCookie` bez opcji przy logout**~~ ✅ ZROBIONE (2026-07-15) — wyciągnięty helper `getCookieOptions(): CookieOptions` w `userController.ts` (zwraca `httpOnly`/`sameSite`/`secure` zależne od `NODE_ENV`); `login` używa `{ ...getCookieOptions(), maxAge }`, `logout` — `clearCookie('token', getCookieOptions())`. Jedno źródło prawdy → atrybuty login/logout nie mogą się rozjechać. `maxAge` świadomie NIE w helperze (specyficzny dla logowania; clearCookie kasuje przez wygaszenie). Adnotacja `: CookieOptions` zapobiega poszerzeniu `sameSite` do `string`. Uwaga: to była kosmetyka (kasowanie i tak działało — path domyślnie `/`), fix to zabezpieczenie na przyszłość, gdyby ktoś zmienił `path`/`domain` po jednej stronie.
...
- ~~**F8 — Modal a11y**~~ ✅ ZROBIONE (2026-07-17) — `Modal.tsx` dostał komplet: **Escape** (listener na `document`, bo focus może siedzieć gdziekolwiek), **focus management** (`document.activeElement` zapamiętany w `useRef` **przed** przeniesieniem, przywracany w cleanupie), **focus trap** (Tab/Shift+Tab zawijane wyłącznie na krawędziach — w środku przeglądarka radzi sobie sama; panel liczy się jako krawędź wiodąca, bo ma `tabIndex={-1}` i po otwarciu trzyma focus poza kolejką Taba), **scroll lock** (`body.style.overflow`), `role="dialog"` + `aria-modal="true"` + **`aria-labelledby`**. Cztery osobne effecty (focus / scroll / trap / Escape), każdy ze strażnikiem `if (!open) return` i cleanupem. Nazwa dialogu: `useId()` u wywołującego (`Search`, `AllProducts`) → `labelledBy` na `Modal` + `titleId` na `ProductForm`, który kładzie je na swoim `<h1>`. Świadomie **nie** props `title` na Modalu — `ProductForm` renderuje się też **poza** modalem (`/dashboard/add`) i tam nagłówka potrzebuje; oba propsy opcjonalne, `undefined` → React pomija atrybut.
  **Pułapki (każda kosztowała osobno):**
  1. `document.activeElement` w **ciele komponentu** wywalił `next build`: `ReferenceError: document is not defined` przy prerenderze `/dashboard/all`. `"use client"` NIE znaczy „tylko w przeglądarce" — komponent i tak leci SSR-em. `document`/`window` wolno dotykać wyłącznie w effektach i handlerach. (Strażnik `document ? ... : ...` **nie działa** — odczyt nieistniejącej zmiennej rzuca `ReferenceError`, zanim `?` cokolwiek oceni; jedyny bezpieczny to `typeof`.)
  2. `useRef(document.activeElement)` — argument `useRef` jest czytany **tylko przy pierwszym renderze**, więc zapisałby focus z montowania strony (`<body>`), nie z otwarcia modala. Ref napełnia się **w effekcie**, nie w deklaracji.
  3. `useState` zamiast `useRef` na „kto miał focus" = **stale closure**: cleanup domyka się nad wartością z tamtego renderu (`null`), focus nigdy nie wraca. Ref to wspólne pudełko dla wszystkich renderów.
  4. `labelledBy="titleId"` (cudzysłów zamiast klamer) **przechodzi `tsc`** — typ propsa to `string`, a literał `"titleId"` jest stringiem. W runtime `aria-labelledby` wskazuje w próżnię i dialog zostaje bez nazwy. Ta sama klasa co **B11**: typ sprawdza kształt, nie sens.
  5. `body.style.overflow = "auto"` w cleanupie **zgaduje** oryginał (`globals.css` nie ma reguły na `body`, czyli `visible`) i zostawia inline style bijący arkusz na zawsze. `= ""` **zdejmuje** nadpisanie zamiast wstawiać nowe — ta sama zasada „zapisz albo cofnij, nigdy nie zgaduj", co przy focusie.
  6. Lista focusowalnych liczona **w handlerze** przy naciśnięciu Tab, nie przy renderze/otwarciu — inaczej rozjeżdża się, gdy zawartość modala doładuje się z API. Selektor jako stała modułowa (`FOCUSABLE_SELECTOR`), bo jest niezmienny — `useMemo` byłby tu zbędny i tak by nie trafiał (zależność zmieniająca tożsamość co render; ESLint sam to zgłosił, ta sama pułapka co `useCallback` z `[onClose]`).
  **Lekcja:** `tsc` i `build` sprawdzają, czy się kompiluje — nie czy focus krąży. Trap i nazwa dialogu weryfikowalne tylko klawiaturą + DevTools → Accessibility → Name. Zweryfikowane ręcznie na `/dashboard/all`.
...
7. ~~**B3, B4, B5 + F2-check**~~ ✅ zrobione — wszystkie trzy domknięte (B3 = decyzja „bez `pino`", patrz dług techniczny → Backend)
