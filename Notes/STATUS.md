...
- ~~**B3 — Usuń `console.log` z produkcji**~~ ⚠️ NOTATKA NIEAKTUALNA (zweryfikowane 2026-07-15) — wskazane `console.log` (`productController` ~99, `diaryService` ~134/153) **już nie istnieją**. W całym `src` zostały tylko dwa `console.`: `index.ts:9` (log startu serwera, standard) i `errorHandler.ts:12` (`console.error` w handlerze — właściwe miejsce). Czyli „usuń console.log z kontrolerów" jest bezprzedmiotowe. **Zostaje osobno:** świadoma decyzja „czy wprowadzamy `pino`" (to nowy feature/logger, nie sprzątanie długu). **Lekcja (ta sama co B5/B10):** notatka TODO się zdezaktualizowała — zweryfikuj gerpem realny stan, zanim „naprawisz".
